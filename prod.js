...
function igor(collection) {
    ...
    for (let [index, value] of collection.entries()) {
        if (index == 0) {
            ...
            if (newdata[newdata.length - 1].product === value.productID) {
                newdata[newdata.length - 1].options = Object.assign(newdata[newdata.length - 1].options, {
                    [value.option_En]: value.option_value
                })
            } else {
                newdata.push({
